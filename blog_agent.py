...
# Streamlit UI
st.title("AI Blog Generator Agent")
app = create_workflow()
...
keyword = st.text_input("Enter blog topic keyword:", value=st.session_state.blog_state["keyword"])

if st.button("Generate Titles") and keyword.strip():
    with st.spinner("Generating title options..."):
        new_state = app.invoke({"keyword": keyword.strip(), "titles": [], "selected_title": None, "blog_content": None})
        st.session_state.blog_state.update(new_state)

if st.session_state.blog_state["titles"]:
    st.subheader("Generated Titles")
    selected_idx = st.radio("Select a title:", options=range(len(st.session_state.blog_state["titles"])),
...
if st.session_state.blog_state["blog_content"]:
    ...
    st.download_button("Download Markdown", st.session_state.blog_state["blog_content"], file_name=f"{st.session_state.blog_state['selected_title']}.md")
